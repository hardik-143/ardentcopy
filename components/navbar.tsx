...
/** Normalize category to a single group key (e.g. "Georgia - Alpharetta" → "georgia"). */
function categoryGroupKey(category: string | null | undefined): string | null {
  const raw = typeof category === "string" ? category.trim() : "";
  if (!raw) return null;
  const firstPart = raw.split(/[\s]*[-–—]\s*|[,\s]+/)[0]?.trim() ?? raw;
  return firstPart.toLowerCase() || null;
}

/** Display label for a category (e.g. "Georgia - Alpharetta" → "Georgia"). */
...
/** Groups column links by category. Each category label is shown once with all its links under it. */
export function groupLinksByCategory(links: ColumnLink[] | undefined) {
  if (!links?.length) return [];
  const categoryOrder: (string | null)[] = [];
  const byCategory = new Map<string | null, ColumnLink[]>();
  const displayLabelByKey = new Map<string | null, string | null>();
  for (const link of links) {
    const raw = link.category != null ? stegaClean(link.category) : null;
    const key = categoryGroupKey(raw);
    const displayLabel = categoryDisplayLabel(raw);
    if (!byCategory.has(key)) {
      categoryOrder.push(key);
      byCategory.set(key, []);
      displayLabelByKey.set(key, displayLabel);
    }
    byCategory.get(key)?.push(link);
  }
  return categoryOrder.map((key) => ({
    category: displayLabelByKey.get(key) ?? null,
    links: byCategory.get(key) ?? [],
  }));
}
...
function DesktopColumnDropdown({
  ...
  const grouped = groupLinksByCategory(column.links);
  ...
  return (
    <div className="group relative">
      <button
        ...
        aria-haspopup="menu"
        className="flex items-center gap-1 px-3 py-2 font-light text-md transition-colors hover:text-foreground"
      ...
      {isOpen ? (
        ...
        >
          <div className="grid gap-3">
            {grouped.map(({ category, links: groupLinks }, i) => (
              <div
              ...
              >
                {category ? (
                  <div
                    className="px-3 py-1.5 font-medium text-primary text-xs uppercase tracking-wider"
                    role="presentation"
                  >
                    {category}
                  </div>
      ...
      <div className="container mx-auto px-4">
        <div className="flex h-16 lg:h-24 items-center justify-between">
          ...
          <div className="hidden items-center gap-4 lg:flex">
            {phoneNumber ? (
              <a
                className="font-bold text-2xl transition-colors text-primary hover:text-primary/70"
          ...
          <div className="flex items-center gap-2 lg:hidden">
            <a
              href={`tel:${phoneNumber?.replace(/\s/g, "")}`}
              className="text-xs text-white bg-primary px-3 py-2 rounded-sm font-bold"
            >
              CALL NOW
            </a>
