...
const DOCUMENT_TYPE_CONFIGS: Record<string, SlugValidationOptions> = {
  ...
  blog: {
    ...
    customValidators: [
      (slug: string) => {
        ...
        if (
          segments.length === 2 &&
          (segments[1]?.length ?? 0) < MIN_BLOG_SLUG_LENGTH
        ) {
  ...
  glossaryIndex: {
    documentType: "Glossary page",
    requiredPrefix: "/glossary",
    requireSlash: true,
    segmentCount: 1,
    sanityDocumentType: "glossaryIndex",
    forbiddenPatterns: [/^\/glossary\/.+/],
    customValidators: [
      (slug: string) => {
        if (slug !== "/glossary") {
          return ["Glossary page must be exactly '/glossary'"];
        }
        return [];
      },
    ],
  ...
  careersIndex: {
    documentType: "Careers page",
    requiredPrefix: "/careers",
    requireSlash: true,
    segmentCount: 1,
    sanityDocumentType: "careersIndex",
    forbiddenPatterns: [/^\/careers\/.+/],
    customValidators: [
      (slug: string) => {
        if (slug !== "/careers") {
          return ["Careers page must be exactly '/careers'"];
        }
        return [];
      },
    ],
