...
function printSeedDataImportInstructions() {
  ...
  const lines = [
    ...
    padLine(),
    padLine(
      `${COLOR_BLUE}${styleCommand("cd studio && npx sanity dataset import seed-data.tar.gz <TARGET_DATASET>")}${COLOR_RESET}`
    ),
    padLine(),
    padLine(`${COLOR_BLUE}Example:${COLOR_RESET}`),
    padLine(
      `${COLOR_BLUE}${styleCommand("cd studio && npx sanity dataset import seed-data.tar.gz production --replace")}${COLOR_RESET}`
    ),
    padLine(),
    `└${"─".repeat(BOX_WIDTH)}┘`,
    "",
